...
# sample number
n_samples_list = range(100,600,+100)
# 初始化准确度列表
bic_accuracy = np.zeros(len(n_samples_list))
aic_accuracy = np.zeros(len(n_samples_list))
vbem_accuracy = np.zeros(len(n_samples_list))
...
for i, (method, accuracy) in enumerate(zip(methods, accuracy_list)):
    plt.plot(n_samples_list, accuracy, label=method, marker='o')

plt.xlabel("n_samples_list")
plt.ylabel("accuracy")
plt.title("accuracy-n_samples_list")
plt.legend()
plt.show()